            ...
            <form class="form-inline" method="POST" action="{{ route('purchase.store') }}">
                ...
                <div class="row">
                    <div class="col-sm-2 text-left">
                        <label>Purchase ID: {{ $bill_id }}</label>
                    </div>
        ...
        $(document).ready(function () {
            $('#item_code').change(function () {
                ...
                if (item_code != '') {
                    $.ajax({
                        type: 'GET',
                        url: '{{ url('purchase/find-item-by-item-code') }}' + '/' + item_code,
                        error: function () {
                            console.log('error......');
                        },
            ...
            $('#item_name').change(function () {
                ...
                if (item_name != '') {
                    $.ajax({
                        type: 'GET',
                        url: '{{ url('purchase/find-item-by-item-name') }}' + '/' + item_name,
                        error: function () {
                            console.log('error......');
                        },
                        success: function (data) {
            ...
            $('#payment_mode').change(function () {
                ...
                $.ajax({
                    type: 'GET',
                    url: '{{ url('purchase/payment-mode') }}' + '/' + payment_mode,
                    error: function () {
                        console.log('error in payment mode');
                    },
                    success: function (data) {
                        $('#select_supplier').html(data);
                    }
                });
            });
